...
{ 
  ...
  if (LittleFS.exists("/config.txt")) {
    ...
    if (file) {
      String fileContent = "";
      char buffer[512 + 1];  // +1 pour le caract�re nul (terminaison de cha�ne)
      size_t bytesRead = 0;

      while (file.available()) {
        bytesRead = file.readBytes(buffer, 512);
        buffer[bytesRead] = '\0';  // Assurez-vous que la cha�ne est termin�e
        fileContent += buffer;
      }
      file.close();
...
String  SamcoPreferences::structuresToJson() {
    // Cr�er un document JSON dynamique
    ...
    // Convertir le document JSON en cha�ne
    String jsonString;
...
bool SamcoPreferences::JsonToStructures(const String& jsonString) {
    ...
    // D�s�rialiser la cha�ne JSON
    DeserializationError error = deserializeJson(doc, jsonString);
    if (error) {
        return false; // Erreur de d�s�rialisation
    }
    ...
    if (!profilesJson.isNull()) {
        for (uint8_t i = 0; i < profilesJson.size() && i < SamcoPreferences::profiles.profileCount; ++i) {
            ...
            char hexColor[8]; // La cha�ne hexad�cimale pour la couleur (#RRGGBB)
            ...
            if (profileJson.containsKey("color")){
                ...
                // Assurez-vous que la cha�ne est bien format�e en hex (par exemple #RRGGBB)
                if (colorStr[0] == '#') {
                    uint32_t color;
                    sscanf(colorStr + 1, "%6x", &color); // Ignorez le caract�re '#' et lisez un hex
                    profile.color = color;
                }
            }
